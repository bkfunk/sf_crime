  ...
  function brushMove() {
    ...
    if (orientation === "horizontal") {
        ...
        highlightClipPathRect.attr("y", yScale(extent[1]))
              .attr("height", yScale(0) - yScale(extent[1] - extent[0]));
    }
...
/* Public functions */
  ...
  plot.updatePlot = function() {
    ...
    if (orientation === "horizontal") {
      brush.x(xScale)
            .extent(d3.extent(data, (orientation === "horizontal") ? value : densityValue));
    } else {
      brush.y(yScale)
            .extent(d3.extent(data, (orientation === "horizontal") ? densityValue : value));
    }
  ...
  plot.updateAxes = function(axes) {
    ...
    // Update y-axis
    if (axes.indexOf("y") !== -1) {
      console.log((orientation === "horizontal") ? densityValue : value);
      yScale.domain(
  ...
  /*
  plot.brushDirection = function(_) {
    if (!arguments.length) return brushDirection;
    brushDirection = _;
    if (plotCreated) brushg.selectAll(".resize")
        .attr("style", "cursor: " + brushDirection + "-resize;");
    return plot;
  };
  plot.brushAxis = function(_) {
    if (!arguments.length) return brushAxis;
    brushAxis = _;
